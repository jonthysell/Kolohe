...
namespace Kolohe.Test
{
    public class TestUtils
    {
        ...
        public static void LoadAndExecuteTestCases<T>([CallerMemberName] string fileName = "") where T : ITestCase, new()
        {
        ...
        private static Stream GetEmbeddedResource(string fileName)
        {
            ...
            foreach (var name in TestAssembly.GetManifestResourceNames())
            {
                ...
                {
                    return TestAssembly.GetManifestResourceStream(name) ?? throw new Exception($"Unable to get \"{name}\"");
                }
            }
